...
class XMLHelper:
  def _pretty_print(self, current, parent=None, index=-1, depth=0):
    for i, node in enumerate(current):
      XMLHelper._pretty_print(self, node, current, i, depth + 1)
  ...
  def store_xml(self, root, path):
    XMLHelper._pretty_print(self, root)
    tree = ElementTree(root)
    with open(path, "wb") as file:
      tree.write(file, encoding='utf-8', xml_declaration=True)


  def build_xml_commit_info(self, root, curr_comm:Commit, prev_comm:Commit, files_info):
  ...
  def build_xml_files_info(self, root, repo, curr_comm:Commit, prev_comm:Commit, files_info):
  ...
  def build_xml_info(self, repo, curr_comm:Commit, prev_comm:Commit, files_info):
    root = Element("Info")
    root = XMLHelper.build_xml_commit_info(self, root, curr_comm, prev_comm, files_info)
    root = XMLHelper.build_xml_files_info(self, root, repo, curr_comm, prev_comm, files_info)
